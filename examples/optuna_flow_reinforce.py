...
def worker_process(journal_path, study_name):
    ...
    try:
        ...
        def objective(trial):            
            ...
            mu_lr = trial.suggest_float("mu_lr", 0.1, 10.0, log=True)
            ...
            run_name = f"flow_reinforce_t{trial.number}_tgt{lora_perturb_target}_lr{flow_lr:.1e}_ent{alpha_entropy:.1e}_sig{target_sigma:.2f}"
            ...
            trainer = OptunaFlowTrainer(
                ...
                ppo_mode=False,
                wandb_project="propagate_flow_reinforce_sweeps",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Optuna Sweep for FlowES (REINFORCE)")
    parser.add_argument("--trials", type=int, default=5, help="Total trials to run on this node")
    args = parser.parse_args()

    JOURNAL_PATH = "sweep_flow_reinforce.journal"
    STUDY_NAME = "study_flow_reinforce"

    study = get_or_create_study(STUDY_NAME, JOURNAL_PATH)

    print(f"\n#--- Starting FlowES REINFORCE Sweep ({args.trials} trials) ---#")
