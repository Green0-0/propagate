...
try:
    ...
    update_chain = [chain.Init_Perturbation_Gaussian(), chain_log.Log_Perturb_Norms(), chain_adam.OC_Compute_RMSProp_Blockwise(), chain.Zero_Perturb_Buffer(), chain_adam_seeded.OC_Update_Seed_History(), chain_adam_seeded.OC_Apply_Momentum_Seeded(), chain.Sign_Perturb_Buffer(), chain.Scale_Perturbation(div_by_pop=True, mul_by_lr=True, div_by_rmsprop_block=True), chain_misc.Probabilistic_Zero_Perturb_Buffer(), chain.Add_Perturb_Buffer(), chain.Delete_Perturb_Buffer(), chain_log.Log_RMSProp_Norms()]
    optimizer = Optimizer(optimizer_name="Test Optimizer", total_steps=200, learning_rate=20, perturb_scale=0.02, population_size=14, perturb_chain=perturb_chain, update_chain=update_chain, norm_by_mean=False)
