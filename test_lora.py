...
try:
    ...
    backend = VLLMBackendLoRA(model_name="Qwen/Qwen2.5-3B-Instruct", NUM_GPUS=1, CPUS_PER_GPU=12, GPU_FRACTION_VLLM_WORKER=0.7, Sampler=sampler, population_size=3, lora_rank=16, use_tqdm=False, time_self=True)
    
    optimizer = SimpleOptimizer(total_steps=150, learning_rate=0.002, seed_weight=0.004)
    ...
    trainer = SimpleTrainer(population_size=3,
                            mirror=False,
                            optimizer=optimizer,
                            backend=backend,
                            dataset=dataset,
