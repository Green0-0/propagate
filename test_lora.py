...
try:
    dataset = load_countdown_dataset(batch_size=150)
    ...
    backend = VLLMBackendLoRA(model_name="Qwen/Qwen2.5-3B-Instruct", NUM_GPUS=4, CPUS_PER_GPU=6, GPU_FRACTION_VLLM_WORKER=0.7, Sampler=sampler, population_size=12, lora_rank=128, use_tqdm=False, time_self=True)
    
    optimizer = SimpleOptimizer(total_steps=150, learning_rate=0.015, seed_weight=0.05)
