...
class VLLMBackendLoRA(Backend):
    def __init__(self, 
                 ...
                 lora_model_source: str = None, 
                 norm_scale_update: bool = True, 
                 repeat_tokens_buffer_count: int = 20, 
                 repeat_times_kill: int = 15, 
                 rep_check_every: int = 100, 
                 repeat_tokens_begin_scan_count: int = 500, 
                 repeat_tokens_lookback_length: int = 500):
        ...
        self.max_loras_per_gpu = max_loras_per_gpu
    ...
    def startup(self, trainer: SimpleTrainer):
        ...
        print(f"#-- Max LoRAs per GPU: {self.max_loras_per_gpu} (Fixed Slot Mode) --#")
        ...
        max_loras_on_engine = self.max_loras_per_gpu
        ...
        self.inference_engines = [
            ray.remote(
                ...
                enable_lora=True,
                max_loras=max_loras_on_engine,  # Reduced memory footprint
        ...
        print("#-- Creating LoRA adapters (Slots) --#")
        ...
        if self.init_lora_weights == "zero":
            ...
            lora_cfg = LoraConfig(r=self.lora_rank, lora_alpha=1, target_modules=default_target_modules, use_rslora=True, init_lora_weights=False)
            peft_model = get_peft_model(base_model, lora_cfg)
            with torch.no_grad():
                for name, param in peft_model.named_parameters():
                    if "lora_" in name: param.data.zero_()
             ...
             lora_cfg = LoraConfig(r=self.lora_rank, lora_alpha=1, use_rslora=True, target_modules=default_target_modules, init_lora_weights=False)
             peft_model = get_peft_model(base_model, lora_cfg)
             ...
             lora_cfg = LoraConfig(r=self.lora_rank, lora_alpha=1, target_modules=default_target_modules, init_lora_weights=self.init_lora_weights, use_rslora=True)
             peft_model = get_peft_model(base_model, lora_cfg)

        self._lora_tmp_root = tempfile.mkdtemp(prefix="vllm_loras_")

        lora_names = [f"lora_{i}" for i in range(self.max_loras_per_gpu)]
        self.lora_paths = {} 
        ...
        print(f"#-- Preloading {len(lora_names)} LoRA adapter SLOTS into {len(self.inference_engines)} engine(s) --#")
    ...
    def generate_outputs(self, genomes: List[Genome], suffix: str, inputs: List[List[Dict[str, str]]]):
        ...
        for i in inputs:
            ...
            all_prompts.append(prompt_genome)

        if self.time_self:
            start_time = time.time()

        all_work_items = list(zip(genomes, all_prompts))
        total_items = len(all_work_items)
        batch_size = self.max_loras_per_gpu
        batches = [all_work_items[i:i + batch_size] for i in range(0, total_items, batch_size)]
        
        batch_iter = iter(batches)
        
        free_workers = list(range(len(self.inference_engines)))
        inflight = {} 
        
        completed_batches = 0
        total_batches = len(batches)
        
        print(f"#-- Starting Generation: {total_items} items in {total_batches} batches on {self.NUM_GPUS} GPUs --#")
        ...
        while True:
            while free_workers:
                ...
                worker_idx = free_workers.pop(0)
                llm = self.inference_engines[worker_idx]
                
                b_genomes, b_prompts = zip(*batch_data)

                ray.get(llm.collective_rpc.remote(
                    "perturb_self_weights_multi", 
                    args=(np.array(b_genomes).tolist(), self.lora_perturb_target,)
                ))

                current_lora_specs = []
                for i in range(len(b_genomes)):
                    slot_name = f"lora_{i}"
                    current_lora_specs.append({
                        "name": slot_name,
                        "id": i + 1,
                        "path": self.lora_paths[slot_name]
                    })
                
                handle = llm.generate_multi_lora.remote(
                    b_prompts,
                    self.sampler,
                    current_lora_specs,
                    self.use_tqdm
            ...
            done_handles, _ = ray.wait(list(inflight.keys()), num_returns=1)
            finished_handle = done_handles[0]
            
            job_info = inflight.pop(finished_handle)
            w_idx = job_info["worker_idx"]
            b_genomes = job_info["genomes"]
            llm = self.inference_engines[w_idx]
            
            outputs = ray.get(finished_handle)
            
            for genome, gen_outputs in zip(b_genomes, outputs):
                genome.latest_outputs = gen_outputs

            ray.get(llm.collective_rpc.remote(
                "restore_self_weights_multi", 
                args=(np.array(b_genomes).tolist(), self.lora_perturb_target,)
            ))
            
            free_workers.append(w_idx)
            completed_batches += 1
        ...
        if self.time_self:
            end_time = time.time()
            print(f"#-- All genome outputs generated in {end_time - start_time:.2f} seconds --#")
